...
def sintgrav(R,Z,DRHO,x):
    dsin=(2)*np.pi*(R**2)*DRHO*G*(Z/(x[:]**2+Z**2))
    return dsin
def obsgrav(R0,Z0,DRHO_0,x):
    dobs=(2)*np.pi*(R0**2)*DRHO_0*G*(Z0/(x[:]**2+Z0**2))
    return dobs

def jac(R0,Z0,DRHO_0,x):
    dR=4*np.pi*(R0)*DRHO_0*G*(Z0/(x[:]**2+Z0**2)#dR
    dZ=2*np.pi*(R0**3)*DRHO_0*G
    dRHO=2*np.pi*(R0**2)*G*(Z0/(x[:]**2+Z0**2))#drho
    return dR,dZ,dRHO
